...
extension Program {
    ...
    /// Built-in that returns the key of a node; retuns .null if node dones not have a key
    ...
    /// Built-in that returns the sib of a node; returns .null of it is nil or has no sib.
    func bltinSib(_ args: [ParsedExpr]) async throws -> ProgramValue {
        let node = try await evaluateGedcomNodeOpt(args[0], errMsg: "sibling: arg must be a node")
    ...
    /// Built-in that returns the dad of a node; returns .null if it is nil or has no dad.
    func bltinDad(_ args: [ParsedExpr]) async throws -> ProgramValue {
        let node = try await evaluateGedcomNodeOpt(args[0], errMsg: "dad: arg must be a node")
    ...
    /// Built-in that returns the first kid of a node that has a given tag.
    /// kidwithtag(node, string) -> node?
    func bltinKidWithTag(_ args: [ParsedExpr]) async throws -> ProgramValue {

        guard let node =
            try await evaluateGedcomNodeOpt(args[0], errMsg: "kidwithtag: 1st arg must be a node") else {
            return .null
        }
        let tag = try await evaluateString(args[1], errMsg: "kidwithtag: 2nd arg must be a tag string")
    ...
    /// Built-in that returns list of nodes that are kids of the given node and have a given tag.
    /// kidswithtag(node, string) -> list
