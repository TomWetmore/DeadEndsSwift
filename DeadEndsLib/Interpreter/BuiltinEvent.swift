...
extension Program {
    ...
    func bltinDate(_ arg: [ParsedExpr]) async throws -> ProgramValue {
        let node = try await evaluateGedcomNodeOpt(arg[0], errMsg: "date: arg must be a node")
    ...
    func bltinPlace(_ arg: [ParsedExpr]) async throws -> ProgramValue {
        let node = try await evaluateGedcomNodeOpt(arg[0], errMsg: "place: arg must be a node")
