...
extension Program {
    ...
    func bltinBirth(_ args: [ParsedExpr]) throws -> ProgramValue {
        let person = try evaluatePerson(args[0], errMsg: "birth: arg must be a person")
    ...
    func bltinDeath(_ args: [ParsedExpr]) throws -> ProgramValue {
        let person = try evaluatePerson(args[0], errMsg: "death: arg must be a person")
    ...
    func builtinBurial(_ args: [ParsedExpr]) throws -> ProgramValue {
        let person = try evaluatePerson(args[0], errMsg: "burial: arg must be a person")
